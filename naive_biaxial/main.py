...
def generate(model, pieces, save_name, batch_size=10, length=100):
    ...
    for i in pbar:
        ...
        time_input = np.array(
            [noteStateMatrixToInputForm(p_)
             for p_ in np.expand_dims(step_composition, axis=1)]
        )
...
if __name__ == '__main__':
    ...
    # train(m, pcs, 25000, "model/big/model_")
    generate(m, pcs, "model/big/model_0.006861227")
